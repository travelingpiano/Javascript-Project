...
document.getElementsByName('default')[0].addEventListener('click',()=>{
  type = 'default';
  createCirclePack2();
});

let createCirclePack = ()=>{
  d3.json('data/fortune100.json',(error,root)=>{
    ...
    root = d3.hierarchy(root)
             .sum(d=>{
               return d.Revenue; })
             .sort((a,b)=>(b.Revenue-a.Revenue)); //hierarchy, sum and then root acts as a sort method to sort all the child and parent nodes, in this case, from big to small. these needs to be called before
