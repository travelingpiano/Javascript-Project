...
var createCirclePack = function createCirclePack(filename) {
  d3.json(filename, function (error, root) {
    ...
    var circle = g.selectAll("circle").data(nodes).enter().append('circle').attr('class', function (d) {
      ...
      customTooltip.transition().duration(4000).style('opacity', 0);
    });
    var text = g.selectAll('text').data(nodes).enter().append('text').attr('class', 'label').attr('fill-opacity', function (d) {
      return d.parent === root ? 1 : 0;
    }).attr('display', function (d) {
      return d.parent === root ? 'inline' : 'none';
    }).attr('dy', '0.3em').attr('text-align', 'center').attr('color', 'blue').text(function (d) {
      if (d.children) {
