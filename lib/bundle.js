...
document.getElementsByName('fortune')[0].addEventListener('click', function () {
  type = 'fortune';
  createCirclePack('data/fortune100.json');
});

document.getElementsByName('movies')[0].addEventListener('click', function () {
  type = 'movies';
  createCirclePack('data/movies.json');
});

var customTooltip = d3.select('body').append('textarea').attr('class', 'toolTip').style('opacity', 0);

var createCirclePack = function createCirclePack(filename) {
  d3.json(filename, function (error, root) {
    d3.selectAll('.chart > *').remove();
    var body = d3.select('.chart');
    var width = Number(body.style('width').slice(0, -2));
    var height = Number(body.style('height').slice(0, -2));
    if (width > height) {
      width = height;
    } else {
      height = width;
    ...
    var svg = body.append('svg').attr('class', 'chart').attr('width', width).attr('height', height),
        margin = 20;
    var diameter = +svg.attr('height');
    var g = svg.append('g').attr('transform', 'translate(' + diameter / 2 + ',' + diameter / 2 + ')'),
        //translate to 2, 2 location from default 0,0 location, very minor move
    format = d3.format(',d'); //format number with 0 decimal places
    var pack = d3.pack().size([diameter - margin, diameter - margin]).padding(2); //create new d3 circle packing, size sets the size of the pack by width and height
    if (error) throw error;
    root = d3.hierarchy(root).sum(function (d) {
      return d.Revenue;
    }).sort(function (a, b) {
      return b.value - a.value;
    }); //hierarchy, sum and then root acts as a sort method to sort all the child and parent nodes, in this case, from big to small. these needs to be called before
    var color = d3.scaleLinear().domain([-1, 5]).range(['rgb(38,143,56)', 'rgb(174, 255, 188)']).interpolate(d3.interpolateRgb);
    var focus = root,
    ...
    var circle = g.selectAll("circle").data(nodes).enter().append('circle').attr('class', function (d) {
      ...
      if (d.parent === focus) {
        customTooltip.transition().duration(200).style('opacity', .9);
        customTooltip.html(d.data.name + ' \n ' + d.value + ' millions').style('left', d3.event.clientX + 'px').style('top', d3.event.clientY + 'px');
      } else if (d.parent && d.parent.parent === focus) {
        customTooltip.transition().duration(200).style('opacity', .9);
        customTooltip.html(d.parent.data.name + ' \n ' + d.parent.value + ' millions').style('left', d3.event.clientX + 'px').style('top', d3.event.clientY + 'px');
      }
    }).on('mouseout', function (d) {
      customTooltip.transition().duration(400).style('opacity', 0);
    });
    var text = g.selectAll('text').data(nodes).enter().append('text').attr('class', 'label').style('fill-opacity', function (d) {
      return d.parent === root ? 1 : 0;
    }).style('display', function (d) {
      return d.parent === root ? 'inline' : 'none';
    }).attr('dy', '0.3em').attr('text-align', 'center').attr('pointer-events', 'none').attr('color', 'blue').text(function (d) {
      return d.data.name.substring(0, d.r / 3);
    });
    var node = g.selectAll('circle,text');
    svg.style('background', color(-1)).attr('text-anchor', 'middle').on('click', function () {
      zoom(root);
    });
    ...
    var zoom = function zoom(d) {
      ...
      transition.selectAll('text').style('fill-opacity', function (d) {
      ...
      transition.selectAll('circle').filter(function (d) {
