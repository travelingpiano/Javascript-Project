...
var mainBody = d3.select('.main-body');
var chart = mainBody.append('svg').attr('class', 'chart');
var count = 0;
// let tip = d3.tip()
//             .attr('class','d3-tip')
//             .offest([-10, 0])
//             .html(d=> `Frequency: ${d.value}`);
...
var createBarChart = function createBarChart(data) {
  ...
  }).attr('width', barWidth - 1).on('mouseover', tip.show).on('mouseout', tip.hide);
};

barButton.addEventListener('click', d3.tsv('./data/data.tsv', function (data) {
  createBarChart(data);
}));
